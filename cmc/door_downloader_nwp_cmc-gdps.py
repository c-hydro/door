#!/usr/bin/python3
"""
door - NWP CMC Global Deterministic Forecast System
...
alg_name = 'DOOR - NWP CMC Global Deterministic Forecast System'
alg_version = '1.0.0'
alg_release = '2023-07-03'
...
def main():
    ...
    if model == "GDPS0p15":
        url_blank = "https://dd.weather.gc.ca/model_gem_global/15km/grib2/lat_lon/{run_time}/{step}/CMC_glb_{VAR}_latlon.15x.15_{run_date}{run_time}_P{step}.grib2"
    ...
    if max_step > 240:
        logging.error(" ERROR! Only the first 240 forecast hours are available on the cmc website!")
    time_range = pd.date_range(time_run + pd.Timedelta(str(min_step) + "H"), time_run + pd.Timedelta(str(max_step - 1) + "H"), freq="H")
    forecast_steps = np.arange(3, np.min((max_step+2,240)), 3)
    ...
    for var in variables:
        ...
        out_file = os.path.join(ancillary_out_var, "frc_{step}.grib2")
        ...
        out_files_unzipped = [out_file.format(step=str(t).zfill(3)) for t in forecast_steps]

        inputs = zip(urls, out_files_unzipped)
    ...
    if "APCP_SFC_0" in variables and \
            data_settings['data']['dynamic']["vars_standards"]["decumulate_precipitation"] is True:
        logging.info(" ---> Variable APCP_SFC_0 is cumulated... Performing decumulation")
        first_step = deepcopy(frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]].values[0, :, :])
        frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]] = frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]].diff("time", 1)
        frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]].loc[frc_out.time[0], :, :] = first_step
        frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]] = frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]] / 3
        frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]] = xr.where(frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]]<0,0,frc_out[data_settings['data']['dynamic']["variables"]["APCP_SFC_0"]])

    if "DSWRF_SFC_0" in variables and \
            data_settings['data']['dynamic']["vars_standards"]["decumulate_radiation"] is True:
        logging.info(" ---> Variable DSWRF_SFC_0 is 3-hourly cumulated... Performing decumulation")
        first_step = deepcopy(frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]].values[0, :, :])
        frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]] = frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]].diff("time", 1)
        frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]].loc[frc_out.time[0], :, :] = first_step
        frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]] = frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]] / (3 * 3600)
        frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]] = xr.where(frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]]<0,0,frc_out[data_settings['data']['dynamic']["variables"]["DSWRF_SFC_0"]])

    if "TMP_TGL_2" in variables and \
            data_settings['data']['dynamic']["vars_standards"]["convert_temperature_to_C"] is True:
        logging.info(" ---> Convert temperature to °C...")
        frc_out[data_settings['data']['dynamic']["variables"]["TMP_TGL_2"]] = frc_out[data_settings['data']['dynamic']["variables"]["TMP_TGL_2"]] - 273.15
        frc_out[data_settings['data']['dynamic']["variables"]["TMP_TGL_2"]].attrs['long_name'] = '2 metre temperature'
        frc_out[data_settings['data']['dynamic']["variables"]["TMP_TGL_2"]].attrs['units'] = 'C'
        frc_out[data_settings['data']['dynamic']["variables"]["TMP_TGL_2"]].attrs['standard_name'] = "air_temperature"
        logging.info(" ---> Convert temperature to °C...DONE!")

    if "UGRD_TGL_10" in variables and "VGRD_TGL_10" in variables and \
            data_settings['data']['dynamic']["vars_standards"]["aggregate_wind_components"] is True:
        logging.info(" ---> Aggregate wind components...")
        frc_out["10wind"] = np.sqrt(frc_out[data_settings['data']['dynamic']["variables"]["UGRD_TGL_10"]]**2 + frc_out[data_settings['data']['dynamic']["variables"]["VGRD_TGL_10"]]**2)
