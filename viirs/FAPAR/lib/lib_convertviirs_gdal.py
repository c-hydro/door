#!/usr/bin/env python
#  class to convert/process viirs data using gdal
#
...
##################################################################
#
#  This VIIRS Python class is licensed under the terms of GNU GPL 2.
...
# MOD!! Line 619 skipped: self.write_mosaic_xml(output)
##################################################################
"""Convert VIIRS HDF5 file using GDAL Python bindings. It can create GeoTiff
...
class convertViirsGDAL:
    """A class to convert viirs data from hdf to GDAL formats using GDAL
    ...
    def _reprojectOne(self, l, quiet=True):
        """Reproject a single subset of VIIRS product
    ...
    def run_vrt_separated(self, quiet=True):
...
class file_info:
    ...
    def init_from_name(self, filename):
        ...
        self.projection = PROJ_INFO #[s[1] for s in metadata_list if s[0] == 'Projection'][0]
        self.geotransform = (self.ulx, (self.lrx - self.ulx) / self.xsize, 0,
                                self.uly, 0, (self.lry - self.uly) / self.ysize)

        # xml_tree = ET.parse(xml_filename)
        # root = xml_tree.getroot()

        # # get the coordinates four points at the corners of the image
        # points = root.find(".//SpatialDomainContainer//GPolygon//Boundary")
        # point_list = points.findall(".//Point")
        # lons = [float(p.find('PointLongitude').text) for p in point_list]
        # lats = [float(p.find('PointLatitude').text) for p in point_list]

        # # transform the coordinates in to the values of GDAL geotransform
        # # (upper left x, w-e pixel resolution, rotation, upper left y,
        # # rotation, n-s pixel resolution)
        # ulx = lons[0]
        # weres = abs(np.mean([lons[1] - lons[0], lons[2] - lons[3]]) / self.xsize) # w-e pixel resolution
        # hrot  = np.arctan(np.mean([lats[1] - lats[0], lats[2] - lats[3]]) / self.xsize) # horizontal rotation

        # uly = lats[0]
        # nsres = abs(np.mean([lats[3] - lats[0], lats[2] - lats[1]]) / self.ysize) # n-s pixel resolution
        # vrot  = np.arctan(np.mean([lons[3] - lons[0], lons[2] - lons[1]]) / self.ysize) # vertical rotation

        # self.geotransform = (ulx, weres, hrot, uly, vrot, nsres)
        # self.projection = PROJ_INFO
...
class createMosaicGDAL:
    """A class to mosaic viirs data from hdf to GDAL formats using GDAL
    ...
    def write_mosaic_xml(self, prefix):
        """Write the XML metadata file for VIIRS mosaic
    ...
    def run(self, output, quiet=True):
        """Create the mosaic
    ...
    def write_vrt(self, output, separate=True, quiet=True):
        """Write VRT file
