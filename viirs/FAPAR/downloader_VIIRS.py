...
"""
DOOR - Tool VIIRS downloader

Operative script for VIIRS product downloading
...
python downloader_VIIRS.py -settings_file configuration.json -time=20210104

Configuration file required (conf_{VIIRSPRODUCT}.json)
...
from lib.viirsDownloaderWget import *
from lib.lib_data_io_tiff import *
from lib.lib_utils_logging import set_logging
from lib.lib_convertviirs_gdal import createMosaicGDAL


# -------------------------------------------------------------------------------------
# Algorithm information
alg_name = 'DOOR - SATELLITE VIIRS'
alg_version = '2.0.1'
...
def main():
    ...
    if end == None:
        ...
        if(args.t == None):
            logging.error(' ==> End date needs to be set in arguments or in json file')
    ...
    while (oDate <= oDateTo):
        ...
        # Get Bounding Box (VIIRS tiles selection)
        ...
        if(len(url_list)==0):
            logging.info(" ==> No VIIRS data for day: " + str(oDate.strftime("%Y-%m-%d")) + " --> MOVE TO NEXT DAY")
